...
大富豪（Daifugo）は非常にゲーム性が高く、AI開発の題材として極めて魅力的なゲームです。しかし、これまではルールベースや模倣学習のものが多く、自己対戦ができる「手軽かつ実用的な速度で強化学習（RL）を試せる大富豪の環境（Env）」がオープンソースで存在しませんでした。

本プロジェクトは、自己対戦できる高速環境の土台をつくることを目的としています。
...
- **ローカルルール（RuleConfigでON/OFF切り替え）:**
  ...
  - Jバック
- **未実装（拡張用の余白）:**
  - 縛り、5飛び、7渡し、スペ3返し等（必要に応じて コードを書き換えて追加してください）。
...
|---|---|---|
| **0** | **Pass (パス)** | パス一択。 |
| **1 ~ 195** | **Group (重ね数字)** | 13個のランク \times 15通りの組み合わせ（4枚のスートから1〜4枚を選ぶ全パターン： 2^4 - 1 = 15 ） = **195通り**。枚数が4枚の時は自動で革命トリガーとなります。 |
| **196 ~ 459** | **Stair (階段)** | 4つの各スート \times 66通りの階段パターン（3枚〜13枚連続の全組み合わせ） = **264通り**。枚数4枚以上で革命トリガーとなります。 |
