...
大富豪（Daifugo）は非常にゲーム性が高く、AI開発の題材として極めて魅力的なゲームです。しかし、これまではルールベースや模倣学習、人間が設定したpolicyベースのものが多く、自己対戦ができる「手軽かつ実用的な速度で強化学習（RL）を試せる大富豪の環境（Env）」がオープンソースでほとんど存在しませんでした。
...
- **ローカルルール（RuleConfigでON/OFF切り替え）:**
  ...
  - Jバック
- **未実装（拡張用の余白）:**
  - 縛り、7渡し、禁止あがり等（必要に応じて コードを書き換えて追加してください）。

## 🃏 Card & Action System Definition 
...
* **生存フラグ (`alive_players: u8`)**: 下位4ビット（`0b1111`）でプレイヤーのゲーム継続状態を表現し、上がったプレイヤーの離脱やゲーム終了判定（残り1人以下）を行います。
...
>図:5飛びで相手をスキップして終盤の期待値上げている

<img src="pngfolder/5飛び_3.png" width="400">

>図:5飛びと8切りの強さの違い理解

基本的に5を終盤まで温存して、終盤に親番を握る期待値をあげる行動が目立つ。手札が短くなってくるとこちらが親番を取りにくくなってるが、そこで5が残っていると
親番継続しやすいからだと思われる。また8切りと5飛びの違いも理解しており、順番に処理しする挙動が確認できた。

## Todo

・そろそろルールが複雑になってきたのでPERを導入する

・方策ベースのAgentも追加(単純なPPOは自己対戦に向いてないため工夫は必要)
